#!/usr/bin/env python3
"""validate_report.py — target-integrity validator for the evm-token-due-diligence skill.
...
against captured headers, scope chain/role/provenance/runtime status, report identity against
the manifest, check/evidence/finding/rating linkage, and the signing/broadcast/fork declarations.
...
PIN_FUTURE_TOLERANCE_S = 900
PIN_STALE_S = 7 * 24 * 3600
...
_UNSAFE_RE = re.compile(
    r"(?<!not )\b(is|are|its|it is)\s+(completely |totally |fully |100% |perfectly )?(safe|rug[- ]?proof|risk[- ]?free)\b",
    re.IGNORECASE)
...
class Ctx:
    def __init__(self, manifest: dict, manifest_dir: str, report_text: Optional[str], report_path: Optional[str]):
        ...
        self.scope_norm: set = set()
        for sc in _l(manifest.get("scope_addresses")):
            na = _norm_addr(_d(sc).get("address"))
            if na:
...
def check_identity(ctx: Ctx, issues: Issues) -> None:
    ...
    # scope must contain the target as role token on the target chain
    ...
    for i, p in enumerate(_l(m.get("pins"))):
        ...
        try:
            ...
            issues.error("E-PIN-TIME", "captured_header.timestamp is not a hex quantity",
                         f"{path}.captured_header.timestamp")
        if _is_int(ts):
            if p.get("timestamp_utc") != ddcore.iso_utc(ts):
                issues.error("E-PIN-TIME", f"timestamp_utc {p.get('timestamp_utc')!r} != iso_utc(timestamp_unix) "
                             f"{ddcore.iso_utc(ts)}", f"{path}.timestamp_utc")
...
# --------------------------------------------------------------------------------------
# Chains referenced anywhere must be pinned
# --------------------------------------------------------------------------------------
def check_chains(ctx: Ctx, issues: Issues) -> None:
    ...
    for i, f in enumerate(_l(m.get("findings"))):
        ...
        for eid in _l(f.get("evidence_ids")):
            ...
            if ev is None:
                continue  # reported as E-EVIDENCE-DANGLING elsewhere
            if _is_int(ev.get("chain_id")) and ev["chain_id"] not in ctx.pinned_chains:
                issues.error("E-FINDING-CHAIN", f"finding references evidence {eid} on unpinned chain {ev['chain_id']}",
                             f"$.findings[{i}].evidence_ids")
            elif _is_int(ev.get("chain_id")) and _is_int(f.get("chain_id")) and ev["chain_id"] != f["chain_id"]:
                issues.warn("W-FINDING-CROSS-CHAIN", f"finding on chain {f['chain_id']} cites evidence {eid} on chain "
                            f"{ev['chain_id']} (acceptable for bridge findings; confirm intent)",
                            f"$.findings[{i}].evidence_ids")
...
def check_scope(ctx: Ctx, issues: Issues) -> None:
    ...
    rt = _d(_d(ctx.m.get("target")).get("runtime"))
    if rt.get("pin_id") is not None and rt.get("pin_id") not in ctx.pins:
        issues.error("E-EVIDENCE-DANGLING", f"target.runtime.pin_id {rt.get('pin_id')!r} does not exist",
                     "$.target.runtime.pin_id")
...
def parse_frontmatter(text: str) -> tuple[Optional[dict], str]:
    """Return (frontmatter dict or None, body). Frontmatter = leading '---' line, 'key: value' lines, closing '---'."""
    ...
    for idx in range(1, len(lines)):
        ...
        k, v = line.split(":", 1)
        fm[k.strip()] = v.strip()
    return None, text  # no closing delimiter
...
def check_report(ctx: Ctx, issues: Issues) -> None:
    ...
    if ctx.report_text is None:
        issues.error("E-REPORT-MISSING", f"report file not found: {ctx.report_path!r}", "$.report.path")
        return
    want = _lower(rep.get("sha256"))
    got = ddcore.sha256_bytes(ctx.report_text.encode("utf-8"))
    if want and got != want:
        issues.error("E-REPORT-HASH", f"sha256 of report file {got} != manifest.report.sha256 {want}", "$.report.sha256")
    fm, body = parse_frontmatter(ctx.report_text)
    ctx.frontmatter = fm
    ctx.body = body
    if fm is None:
    ...
    if ctx.target_addr is not None and ctx.target_addr not in body.lower():
        issues.error("E-REPORT-TARGET-ABSENT", "target address does not occur in the report body (outside frontmatter)",
                     "report:body")
    ...
    for tok in _ADDR_TOKEN_RE.findall(body):
        ...
        if na not in ctx.scope_norm and na not in seen_unscoped:
            ...
            issues.warn("W-REPORT-UNSCOPED-ADDRESS", f"address {tok} appears in the report but not in scope_addresses",
                        "report:body")
    # verdict language
    mv = re.search(r"^##[ \t]+Verdict[ \t]*$(.*?)(?=^##[ \t]|\Z)", body, re.MULTILINE | re.DOTALL)
    if mv:
        hit = _UNSAFE_RE.search(mv.group(1))
        if hit:
...
def check_checks(ctx: Ctx, issues: Issues) -> None:
    ...
    for i, c in enumerate(checks):
        ...
        seen.add(cid)
        if c.get("surface") not in SURFACES:
            issues.error("E-CHECK-STATUS", f"unknown surface key {c.get('surface')!r}", f"{path}.surface")
        status = c.get("status")
        ev = _l(c.get("evidence_ids"))
        fids = _l(c.get("finding_ids"))
...
def check_evidence(ctx: Ctx, issues: Issues) -> None:
    ...
    for i, e in enumerate(_l(ctx.m.get("evidence"))):
        ...
        elif pid is not None and ctx.pins[pid].get("chain_id") != e.get("chain_id"):
...
def check_findings(ctx: Ctx, issues: Issues) -> None:
    ...
    for i, f in enumerate(_l(ctx.m.get("findings"))):
        ...
        pot = f.get("pin_or_tx")
        if not (isinstance(pot, str) and (pot in ctx.pins or _TX_RE.match(pot))):
        ...
        if f.get("severity") in ("critical", "high") and f.get("confidence") in ("proven", "strongly_supported"):
            types = [_d(ctx.evidence.get(e)).get("evidence_type") for e in ev if e in ctx.evidence]
            if types and all(t in DISCOVERY_EVIDENCE_TYPES for t in types):
                issues.warn("W-EVIDENCE-DISCOVERY-ONLY", f"finding {fid} is {f.get('severity')}/{f.get('confidence')} "
                            f"but every cited evidence is discovery-class ({sorted(set(types))}); material onchain "
...
def check_ratings(ctx: Ctx, issues: Issues) -> None:
    ...
    for key, r in ratings.items():
        r = _d(r)
        path = f"$.ratings.{key}"
        basis = _l(r.get("basis_check_ids"))
        rating = r.get("rating")
        qualified = r.get("coverage_qualified") is True and bool(_s(r.get("coverage_note")).strip())
        unresolved_basis = []
        for cid in basis:
            ...
            if c is None:
                ...
                continue
            if c.get("status") in ("unknown", "skipped"):
                unresolved_basis.append(cid)
            if c.get("status") == "finding" and c.get("severity") == "critical" and rating != "critical":
                issues.error("E-RATING-CRITICAL-AVERAGED", f"rating {key} is {rating!r} but basis check {cid} carries a "
                             "critical finding; a critical finding is never averaged away", f"{path}.rating")
        if rating == "low":
            if not basis:
...
def check_declarations(ctx: Ctx, issues: Issues) -> None:
    ...
    if used is True:
        ...
        if isinstance(ap, str) and ap:
            if not os.path.exists(os.path.join(ctx.dir, ap)):
...
_SEMANTIC_CHECKS = [
    ...
    ("scope", check_scope), ("report", check_report), ("checks", check_checks), ("evidence", check_evidence),
    ("findings", check_findings), ("ratings", check_ratings), ("declarations", check_declarations),
    ("coverage", check_coverage), ("verdict", check_verdict),
]
...
def validate_objects(manifest: Any, report_text: Optional[str], manifest_dir: str = ".", strict: bool = False,
    ...
    if isinstance(manifest, dict):
        ctx = Ctx(manifest, manifest_dir, report_text, report_path)
...
def validate(manifest_path: str, report_path: Optional[str] = None, strict: bool = False) -> dict:
    """Validate manifest file + report file. report_path defaults to manifest.report.path relative to the manifest."""
    ...
    if report_path is None:
        ...
        report_path = os.path.join(manifest_dir, rp) if isinstance(rp, str) and rp else None
    else:
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    ap.add_argument("--report", help="path to the report (default: manifest.report.path relative to the manifest)")
    ap.add_argument("--json", action="store_true", help="print a JSON result instead of human-readable lines")
    ap.add_argument("--strict", action="store_true",
                    help="promote W-REPORT-UNSCOPED-ADDRESS, W-EVIDENCE-DISCOVERY-ONLY and W-VERDICT-LANGUAGE to errors")
    try:
